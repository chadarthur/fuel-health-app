...
import {
  Clock, Flame, Dumbbell, ChefHat, Sparkles, ShoppingCart, Loader2, Plus
} from "lucide-react";
...
function SavedRecipeCard({ recipe, onAddToGrocery, groceryAdded, groceryLoading }: SavedRecipeCardProps) {
  return (
...
export default function SavedRecipesPage() {
  ...
  async function handleAddToGrocery(recipeId: string) {
    if (groceryAddedIds.has(recipeId) || groceryLoadingIds.has(recipeId)) return;
    setGroceryLoadingIds((prev) => new Set([...prev, recipeId]));
    try {
      ...
      setGroceryAddedIds((prev) => new Set([...prev, recipeId]));
    } finally {
      setGroceryLoadingIds((prev) => {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {filtered.map((recipe) => (
              <SavedRecipeCard
                ...
                groceryAdded={groceryAddedIds.has(recipe.id)}
                groceryLoading={groceryLoadingIds.has(recipe.id)}
              />
            ))}
          </div>
        )}
