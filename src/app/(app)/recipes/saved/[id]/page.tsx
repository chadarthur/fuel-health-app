...
export default function SavedRecipeDetailPage() {
  ...
  const [grocerySuccess, setGrocerySuccess] = useState<string | null>(null);
  ...
  function handleGrocerySuccess(added: number, merged: number) {
    const parts: string[] = [];
    if (added > 0) parts.push(`${added} item${added !== 1 ? "s" : ""} added`);
    if (merged > 0) parts.push(`${merged} merged`);
    setGrocerySuccess(parts.join(", ") || "Done");
    setSheetOpen(false);
  }
    ...
    <>
      <div className="min-h-screen bg-background pb-28">
        ...
        <div className="px-4 pt-4 space-y-4">
          ...
          {grocerySuccess ? (
            <div className="w-full py-3.5 rounded-2xl bg-[#00D4AA]/15 border border-[#00D4AA]/30 flex items-center justify-center gap-2">
              <ShoppingCart size={15} className="text-[#00D4AA]" />
              <span className="text-sm font-semibold text-[#00D4AA]">{grocerySuccess} to grocery list!</span>
            </div>
          ) : (
            <Button
              onClick={() => setSheetOpen(true)}
              className={cn(
                "w-full gap-2 font-semibold bg-[#00D4AA] hover:bg-[#00D4AA]/90 text-black"
              )}
            >
              <ShoppingCart size={15} />
              Add to Grocery List
              {recipe.ingredients.length > 0 && (
                <span className="ml-1 text-xs opacity-70">({recipe.ingredients.length} items)</span>
              )}
            </Button>
          )}
